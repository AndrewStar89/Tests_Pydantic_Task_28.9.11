...
from helpers.functions import generate, path_params_prepare, list_data_of_response
from serializers.booking import BookingIdsResponse


@pytest.mark.parametrize('params', [{"firstname": "Sally"}, {"lastname": "Brown"},
                                    {"firstname": "Sally", "lastname": "Brown"}], ids=generate)
...
def test_get_booking_ids_without_params():
    ...
    if len(get_ids.json()) != 0:
        ...
        # если список питомцев пустой, то выдаём исключение с текстом об отсутствии питомцев
